...
function run() {
  ...
  for (let j = 1; j <= results.length; j++) {
    ...
    if (instanceNum.filter((v: number) => v > 2).length === 0) {
      ...
      a++;
    } else if (instanceNum.slice(1, 4).filter((v: number) => v < 3).length === 3) {
      resultsArr.push({
        key: b,
        value: results[i],
      });
  ...
  for (let j = 1; j < resultsArr.length; j++) {
    ...
    if (kk >= 0 && kk < 1500) {
      // if (kk >= 1000 && kk < 2000) {
      // if (kk >= 2000 && kk < 3000) {
      let robatInstance = images(`${inputPath}/${types[0].type}/${instance[0]}.png`)
        .size(1600)
      ...
      db[kk] = {
        name: `Karsier #${kk}`,
        description: 'Karsier came to the blockchain world to create some fun for us.',
        image: `https://karsier.kaco.finance/karsier/image/${kk}.jpeg`,
        attributes: attributes,
      };
      fs.writeFile(path.resolve(__dirname, `../karsier/${kk}.json`), JSON.stringify(db[kk]), function (err: any) {
        if (err) {
          throw err;
        }
      });
  ...
  fs.writeFile(path.resolve(__dirname, '../db/config.json'), JSON.stringify(db), function (err: any) {
    if (err) {
      throw err;
    }
  });
